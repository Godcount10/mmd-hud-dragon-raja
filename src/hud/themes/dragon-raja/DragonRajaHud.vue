...
function notify(result: ActionResult): void {
  feedback.value = { ok: result.ok, message: result.ok ? '动作已提交，等待最新快照' : result.error?.message || '原生动作失败' }
...
function handleLocalAction(action: string): void {
  ...
  else if (action === 'models') void openNative('openModelSettings')
  else if (action === 'settings') surface.value = 'settings'
  else if (action === 'refresh') void refreshConversation()
  else if (action === 'exit') void exit()
}
...
<template>
  <main class="dragon-raja-hud">
    ...
    -->
    <WelcomeSurface v-if="surface === 'welcome'" @enter="surface = 'opening'" />
    <OpeningSurface v-else-if="surface === 'opening'" @complete="completeOpening" @cancel="enterStory" />
    <section v-else class="dr-shell">
      <header class="dr-topbar">
        <button class="dr-brand" type="button" aria-label="返回故事" @click="surface = 'story'"><span class="dr-brand__mark">DR</span><span><strong>Dragon Raja</strong><small>ACADEMY ARCHIVE</small></span></button>
        ...
        <button class="dr-menu-trigger" type="button" @click="menuOpen = true"><i /><i /><span>菜单</span></button>
      </header>
      <div class="dr-shell__rule" />
      <section v-if="surface === 'story'" class="dr-workspace">
        <aside class="dr-sidebar">
          <div><span>WORLD SIGNAL</span><h2>正在发生</h2></div>
          <div class="dr-sidebar__status"><small>AI DERIVED STATUS</small><p v-if="!statuses.length">等待叙事标记……</p><dl v-else><template v-for="item in statuses" :key="item.key"><dt>{{ item.key }}</dt><dd>{{ item.value }}</dd></template></dl></div>
          <nav><button type="button" @click="menuOpen = true"><span>档案终端</span><strong>导航</strong></button><button type="button" @click="void openNative('openModelSettings')"><span>当前模型</span><strong>切换 ↗</strong></button></nav>
