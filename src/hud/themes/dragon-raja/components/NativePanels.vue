...
watch(panel, async (dialog, previous) => {
  if (!dialog) return
  await nextTick()
  if (!previous) panelRoot.value?.focus()
})
...
<template>
  <section v-if="panel" class="dr-native-layer" role="dialog" aria-modal="true" :aria-label="panel">
    ...
    <div ref="panelRoot" class="dr-native-panel" tabindex="-1">
      <header class="dr-native-panel__header">
        <div><span>MMD NATIVE MIRROR</span><h2 v-if="panel === 'instruction'">选择快捷指令</h2><h2 v-else-if="panel === 'edit'">编辑消息</h2><h2 v-else-if="panel === 'model-config'">{{ snapshot.modelConfiguration.title || '模型设置' }}</h2><h2 v-else-if="panel === 'models'">{{ snapshot.modelPanel.title || '模型选择' }}</h2><h2 v-else-if="panel === 'archives'">{{ snapshot.conversationPanel.title || '管理存档' }}</h2><h2 v-else-if="panel === 'persona'">{{ snapshot.personaPanel.title || '用户人设' }}</h2><h2 v-else-if="panel === 'supplement'">{{ snapshot.supplementPanel.title || '补充设定' }}</h2><h2 v-else>{{ snapshot.chatSettings.title || '对话设置' }}</h2></div>
        <button type="button" :disabled="pending !== null || (panel === 'supplement' && snapshot.supplementPanel.picker.open)" @click="close"><svg viewBox="0 0 24 24"><path d="m6 6 12 12M18 6 6 18" /></svg></button>
      </header>

      <main v-if="panel === 'instruction'" class="dr-native-panel__body dr-native-list">
      ...
      <main v-else-if="panel === 'edit'" class="dr-native-panel__body dr-native-edit">
      ...
      <main v-else-if="panel === 'models'" class="dr-native-panel__body">
      ...
      <main v-else-if="panel === 'model-config'" class="dr-native-panel__body">
      ...
      <main v-else-if="panel === 'supplement'" class="dr-native-panel__body dr-native-form">
      ...
      <footer class="dr-native-panel__footer">
        ...
        <button v-if="panel === 'instruction'" type="button" :disabled="pending !== null" @click="close">返回设置</button>
        <button v-else-if="panel === 'edit'" type="button" :disabled="pending !== null" @click="submitEdit">保存修改</button>
        <button v-else-if="panel === 'model-config'" type="button" :disabled="pending !== null" @click="submitModelConfiguration">保存参数</button>
        <button v-else-if="panel === 'archives'" type="button" :disabled="pending !== null" @click="run('createConversation', () => context.invoke('createConversation'))">创建新存档</button>
        <button v-else-if="panel === 'persona'" type="button" :disabled="pending !== null" @click="submitPersona">保存人设</button>
        <button v-else-if="panel === 'supplement'" type="button" :disabled="pending !== null || snapshot.supplementPanel.picker.open" @click="run('submitSupplement', () => context.invoke('submitSupplement', { text: supplementDraft }))">保存补充设定</button>
        <button v-else-if="panel === 'chat-settings'" type="button" :disabled="pending !== null" @click="run('submitChatSettings', () => context.invoke('submitChatSettings'))">确认设置</button>
      </footer>
    </div>
  </section>
</template>
