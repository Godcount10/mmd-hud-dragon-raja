...
const html = pathToFileURL(join(root, 'status-rail-comp-v4.html')).href
...
for (let variant = 1; variant <= 4; variant += 1) {
  ...
  await desktopPage.screenshot({ path: join(root, `status-rail-v4-0${variant}-desktop.png`), fullPage: false })
  await desktop.close()
  ...
  await mobilePage.waitForTimeout(430)
  await mobilePage.screenshot({ path: join(root, `status-rail-v4-0${variant}-mobile-collapsed.png`), fullPage: false })
  await mobilePage.goto(`${html}?variant=${variant}&mobile=open`, { waitUntil: 'load' })
  await mobilePage.waitForFunction(() => window.__compReady === true)
  await mobilePage.waitForTimeout(430)
  await mobilePage.screenshot({ path: join(root, `status-rail-v4-0${variant}-mobile-open.png`), fullPage: false })
  await mobile.close()
}

await browser.close()
console.log('Captured four v4 desktop comps and eight mobile states.')
